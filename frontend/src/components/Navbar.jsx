    ...
    <>
      <header className="fixed inset-x-0 top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
          <Link
            to={user ? "/home" : "/"}
            className="flex flex-shrink-0 items-center gap-2 text-slate-900"
          >
            <img
              src="/insta02.png"
              alt="insta.X logo"
          ...
          {user && (
            <nav className={`${open ? "absolute left-0 right-0 top-16 flex" : "hidden"} md:top-0 w-full md:w-auto flex-col gap-1 border-b border-slate-200 bg-white p-4 md:static md:flex md:flex-row md:items-center md:border-0 md:bg-transparent md:p-0 md:gap-2`}>
              ...
              <button
                onClick={signOut}
                className="flex items-center gap-2 rounded-lg px-3 py-2 text-red-600 transition-colors hover:bg-red-50 md:hidden"
              >
                <LogOut size={18} />
                Log out
              </button>
